...
The Preset selector focuses **Essentials** on the fields needed for a common command while leaving every full category available. General presets include Plain and Multi-GPU; speculative presets include Default Speculative, MTP, DFlash, Draft Model, EAGLE-3, and N-gram; llama-server additionally offers Chat/API, Embeddings, and Reranking.

Presets apply only semantic requirements such as `--spec-type draft-mtp`, `--embedding`, or reranking's `--pooling rank`. They do not guess hardware or performance values, and switching recipes clears only fields owned by the previous or next recipe while preserving unrelated tuning.
...
2. Update `flags.json`, keeping one entry per option group with all aliases, applicable modes, value metadata, serialization rules, and a concise two-to-three-sentence description. Update preset field references when a renamed or replaced flag changes a focused recipe.
