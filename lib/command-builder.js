(function (root, factory) {
    ...
    function buildArguments(registry, state, validation = { errorsById: new Map(), warnings: [] }) {
        ...
        if (filled(values.dflashModel)) {
            const draftModel = registry.byId.get('specDraftModel');
            const specType = registry.byId.get('specType');
            if (draftModel && specType) {
                appendRegistered(segments, draftModel, values.dflashModel);
                appendRegistered(segments, specType, 'draft-dflash');
                skip.add('specDraftModel');
                skip.add('specType');
                const hasDraftConflictWarning = warnings.some(item => [
                    'draft-source-conflict',
                    'dflash-type-override',
                    'dflash-override'
                ].includes(item.id));
                const hasConflictingType = filled(values.specType) && values.specType !== 'draft-dflash';
                if ((filled(values.specDraftModel) || hasConflictingType) && !hasDraftConflictWarning) {
                    warnings.push({
                        id: 'dflash-override',
                        fieldId: 'dflashModel',
                        severity: 'warning',
                        message: 'DFlash model path overrides the generic Draft model path and Spec type fields.'
                    });
                }
            }
        }
